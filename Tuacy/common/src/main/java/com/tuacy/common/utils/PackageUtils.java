...
public class PackageUtils {
	...
	/**
	 * install package silent by root, Attentions: Don't call this on the ui thread, it may costs some times You should add
	 * android.permission.INSTALL_PACKAGES in manifest, so no need to request root permission, if you are system app. Default pm install
	 * params is "-r".
	 *
	 * @param filePath file path of package
	...
	public static int installSilent(Context context, String filePath, String pmParams) {
		 ...
		 **/
		String command = new StringBuilder().append("LD_LIBRARY_PATH=/vendor/lib:/system/lib pm install ")
	...
	/**
	 * uninstall package silent by root, Attentions:Don't call this on the ui thread, it may costs some times. You should add
	 * android.permission.DELETE_PACKAGES in manifest, so no need to request root permission, if you are system app.
	 *
	...
	public static boolean isSystemApplication(Context context, String packageName) {
		...
		return pm != null && isSystemApplication(pm, packageName);
	}

	/**
	 * whether packageName is system application
	 *
	 * @return <ul> <li>if packageManager is null, return false</li> <li>if package name is null or is empty, return false</li> <li>if
	 * package name not exit, return false</li> <li>if package name exit, but not system app, return false</li> <li>else return true</li>
	 * </ul>
	 */
	...
	/**
	 * whether the app whost package's name is packageName is on the top of the stack <ul> <strong>Attentions:</strong> <li>You should add
	 * <strong>android.permission.GET_TASKS</strong> in manifest</li> </ul>
	 *
	...
	/**
	 * Installation return code<br/> a previously installed package of the same name has a different signature than the new package (and the
	 * old package's data was not removed).
	 */
	public static final int INSTALL_FAILED_UPDATE_INCOMPATIBLE = -7;

	/**
	 * Installation return code<br/> the new package is requested a shared user which is already installed on the device and does not have
	 * matching signature.
	 */
	...
	/**
	 * Installation return code<br/> the new package failed while optimizing and validating its dex files, either because there was not
	...
	/**
	 * Installation return code<br/> the new package failed because it contains a content provider with the same authority as a provider
	...
	/**
	 * Installation return code<br/> the package being installed contains native code, but none that is compatible with the the device's
	 * CPU_ABI.
	 */
	...
	/**
	 * Installation return code<br/> the new package couldn't be installed in the specified install location because the media is not
	 * available.
	 */
