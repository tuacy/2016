...
/**
 * SizeConverter.MBTrim.convert(419562f)
 */
public enum SizeConverterUtils {
	...
	/***
	 * <p> 将指定的大小转换到1024范围内的大小. 注意该方法的最大单位为PB, 最小单位为B, 任何超出该范围的单位最终会显示为**. </p>
	 *
	...
	private static final String[] UNITS = new String[]{"B",
													   "KB",
													   "MB",
													   "GB",
													   "TB",
	...
	/***
	 * <p> 存储大小单位间的转换. 注意该方法的最大单位为PB, 最小单位为B, 任何超出该范围的单位最终会显示为**. </p>
	 *
	 * @param unit     从哪个单位开始
	 * @param size     存储大小, 注意是float, 不要以整形的形式传入, 否则会溢出(如:1024*1024这种, 它是先将1024*1024作为int相乘再转换为float的, 如果值过大的话就会溢出了, 所以这么写1024.0F*1024.0F)
	...
	/***
	 * <p> 存储大小单位间的转换, 如果转换后小数部分为0, 则去除小数部分. 注意该方法的最大单位为PB, 最小单位为B, 任何超出该范围的单位最终会显示为**. </p>
	 *
	 * @param unit     从哪个单位开始
	 * @param size     存储大小, 注意是float, 不要以整形的形式传入, 否则会溢出(如:1024*1024这种, 它是先将1024*1024作为int相乘再转换为float的, 如果值过大的话就会溢出了, 所以这么写1024.0F*1024.0F)
	 * @param withUnit 返回的结果字符串是否带有对应的单位
